...
const NSString *DQPlayerCacheFileZoneKey = @"zone";
const NSString *DQPlayerCacheFileSizeKey = @"size";
const NSString *DQPlayerCacheFileResponseHeadersKey = @"responseHeaders";

@interface DQPlayerCacheFile ()
...
@implementation DQPlayerCacheFile
...
- (BOOL)serializeIndex:(NSDictionary *)indexDic
{
    ...
    NSNumber *fileSize = indexDic[DQPlayerCacheFileSizeKey];
    ...
    NSMutableArray *rangeArray = indexDic[DQPlayerCacheFileZoneKey];
    for (NSString *rangeStr in rangeArray) {
        NSRange range = NSRangeFromString(rangeStr);
        [_ranges addObject:[NSValue valueWithRange:range]];
    }

    _responseHeaders = indexDic[DQPlayerCacheFileResponseHeadersKey];
...
- (NSString *)unserializeIndex
{
    ...
    NSMutableDictionary *dict = [@{
                                   DQPlayerCacheFileSizeKey: @(_fileLength),
                                   DQPlayerCacheFileZoneKey: rangeArray
                                   } mutableCopy];
    if (_responseHeaders) {
        dict[DQPlayerCacheFileResponseHeadersKey] = _responseHeaders;
    }
...
- (void)mergeRanges
{
    for (int i = 0; i < _ranges.count; ++i) {
        if ((i + 1) < _ranges.count) {
            ...
            if (DQRangeCanMerge(currentRange, nextRange)) {
...
- (BOOL)setResponse:(NSHTTPURLResponse *)response
{
    ...
    if (![self isFileLengthValid]) {
        success = [self truncateFileWithFileLength:(NSUInteger)[DQPlayerUtils fileLengthOfHTTPURLResponse:response]];
    }
...
- (BOOL)saveData:(NSData *)data atOffset:(NSUInteger)offset synchronize:(BOOL)synchronize
{
    ...
    @try {
        ...
        BOOL flag = [DQPlayerUtils fileHandle:_writeFileHandle safeWriteData:data];
        if (!flag) {
            NSLog(@"write data: %@",@(flag));
        }
    }
    @catch (NSException * e) {
        return NO;
